...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  const TfLiteStackParams* data =
      reinterpret_cast<TfLiteStackParams*>(node->builtin_data);
  ...
  // TODO(Anshuman): Support negative axis.
  ...
  if (input0->type != kTfLiteInt32 && input0->type != kTfLiteFloat32 &&
    ...
    context->ReportError(context, "Type '%s' is not supported by stack.",
  ...
  // Guarantee input/output quantization params match as
  // packing quantized tensors not supported.
...
template <typename T>
void StackImpl(TfLiteContext* context, TfLiteNode* node, TfLiteTensor* output,
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (output->type) {
    case kTfLiteFloat32: {
      StackImpl<float>(context, node, output, data->values_count, data->axis);
      break;
    }
    case kTfLiteUInt8: {
      StackImpl<uint8_t>(context, node, output, data->values_count, data->axis);
      break;
    }
    case kTfLiteInt32: {
      StackImpl<int32_t>(context, node, output, data->values_count, data->axis);
      break;
    }
    case kTfLiteInt64: {
      StackImpl<int64_t>(context, node, output, data->values_count, data->axis);
      break;
    }
    default: {
      context->ReportError(context, "Type '%s' is not supported by stack.",
...
TfLiteRegistration* Register_STACK() {
  static TfLiteRegistration r = {nullptr, nullptr, stack::Prepare, stack::Eval};
  return &r;
}

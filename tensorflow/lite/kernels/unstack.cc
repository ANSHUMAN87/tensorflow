...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type != kTfLiteInt32 && input->type != kTfLiteFloat32) {
    context->ReportError(context,
                         "Currently unstack only supports int32 and float32.");
    return kTfLiteError;
  }
...
template <typename T>
void UnstackImpl(TfLiteContext* context, TfLiteNode* node,
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  switch (input->type) {
    case kTfLiteFloat32: {
      UnstackImpl<float>(context, node, input, data->num, data->axis);
      break;
    }
    case kTfLiteInt32: {
      UnstackImpl<int32_t>(context, node, input, data->num, data->axis);
      break;
    }
    default: {
      context->ReportError(context,
...
TfLiteRegistration* Register_UNSTACK() {
  static TfLiteRegistration r = {unstack::Init, unstack::Free, unstack::Prepare,
                                 unstack::Eval};
  return &r;
}
